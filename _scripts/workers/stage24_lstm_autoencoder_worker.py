...
def log(machine: str, message: str) -> None:
    path = ROOT / "_logs" / machine / "stage24_lstm_autoencoder_worker.log"
...
def read_matrix(asset: str, tf: str, split: str) -> pd.DataFrame:
    path = ROOT / "features" / "learned_inputs" / asset / tf / f"{split}.csv"
    if not path.exists():
...
def make_windows(values: np.ndarray, window: int, max_windows: int | None = None) -> np.ndarray:
    n = len(values) - window + 1
    if n <= 0:
        raise ValueError(f"not enough rows for window={window}")
    if max_windows and n > max_windows:
        starts = np.linspace(0, n - 1, max_windows, dtype=np.int64)
    else:
        starts = np.arange(n, dtype=np.int64)
    out = np.empty((len(starts), window, values.shape[1]), dtype=np.float32)
    for idx, start in enumerate(starts):
        out[idx] = values[start : start + window]
    return out
...
def batched_encode(encoder: Any, values: np.ndarray, timestamps: pd.Series, window: int, batch_size: int) -> pd.DataFrame:
    ...
    out = pd.DataFrame(encoded, columns=[f"learned_lstm_{i:02d}" for i in range(encoded.shape[1])])
...
def run_job(
    ...
    out_dir = ROOT / "features" / "learned_models" / asset / timeframe / "lstm_autoencoder"
    out_dir.mkdir(parents=True, exist_ok=True)
    feature_out = ROOT / "features" / "trading_asset_features" / asset / timeframe / "learned_lstm.parquet"
    ...
    x = tf.keras.layers.LSTM(64, name="encoder_lstm")(inputs)
    latent = tf.keras.layers.Dense(latent_dim, name="latent")(x)
    x = tf.keras.layers.RepeatVector(window)(latent)
    x = tf.keras.layers.LSTM(64, return_sequences=True, name="decoder_lstm")(x)
    outputs = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(len(cols)), name="reconstruction")(x)
    autoencoder = tf.keras.Model(inputs, outputs, name=f"{asset}_{timeframe}_lstm_autoencoder")
    encoder = tf.keras.Model(inputs, latent, name=f"{asset}_{timeframe}_lstm_encoder")
    ...
    metadata = {
        ...
        "note": "Bootstrap Stage 2.4 GPU training while feature-extractor runtime wrapper is being integrated.",
    }
...
def main() -> None:
    ...
    log(args.machine, f"START stage24 lstm asset={args.asset} tf={args.timeframe}")
    ...
    try:
        ...
        release_gpu_lock()
    report_path = ROOT / "_metadata" / f"stage24_lstm_autoencoder_{args.machine}_{args.asset}_{args.timeframe}.json"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
    log(args.machine, f"DONE stage24 lstm asset={args.asset} tf={args.timeframe} val_loss={summary['final_val_loss']:.6f}")
